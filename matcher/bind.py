...
bind = on_command("bind osu", priority=1, block=True)
lock = asyncio.Lock()
...
async def _bind(
    ...
    async with lock:
        if _ := await UserData.get_or_none(user_id=event.get_user_id()):
            await bind.finish(
                v11MessageSegment.reply(event.message_id) + "您已绑定，如需要解绑请输入/bind osu off"
            )
...
unbind = on_command("bind osu off", priority=1, block=True)
